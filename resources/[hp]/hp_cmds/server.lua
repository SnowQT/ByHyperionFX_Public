...
AddEventHandler('chatMessage', function(source, name, message)
        ...
        if string.lower(splitmessage[1]) == "/hjælp" or string.lower(splitmessage[1]) == "/help" then
            ...
            TriggerClientEvent('chatMessage', source, "Hjælp 2: ", {255, 0, 0}, "/mig - Skriv en handling du gør RP-mæssigt")
            TriggerClientEvent('chatMessage', source, "Hjælp 3: ", {255, 0, 0}, "/k - Overgiver dig til politiet, går på knæ med hænderne bag hovedet")
            TriggerClientEvent('chatMessage', source, "Hjælp 5: ", {255, 0, 0}, "/motor - Kan bruges til at tænde og slukke motoren")
            ...
            if nuser_id ~= nil then
                vRP.getUserIdentity({nuser_id, function(identity)
                    if identity then
                        ...
                        TriggerClientEvent('proximityMessage', -1, tonumber(source), "", {185, 217, 224}, 20, fornavn .. " " .. efternavn .. " ** " .. string.sub(message,string.len(splitmessage[1])+1))
                    end
                end})
            end            
        end
