        ...
        vRP.getSData("chest:"..name, function(cdata)
          ...
          local cb_put = function(idname)
            vRP.prompt(source, lang.inventory.chest.put.prompt({vRP.getInventoryItemAmount(user_id, idname)}), "", function(player, amount)
              amount = parseInt(amount)
			       if idname == "mystisk_kasse" or idname == "fbi_harddisk" or idname == "fbi_modem" then
				      TriggerClientEvent("pNotify:SendNotification", source,{text = "Ikke muligt.", type = "warning", queue = "global",timeout = 2000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
             else
              -- weight check
